...
> | A15 second scale | PARTIAL | Qwen3.8-27B (64 layers, 16q/48v grouped linear heads, untied head) chosen and run: gdn/kda/rwkv7/gdn2 build; fp32 exactness vs HF KL 2.6e-6 / top-1 1.0 (bf16 KL 0.066 is HF-vs-FLA kernel rounding compounded over 48 recurrent layers; per-component parity: attention + MLP bit-exact, linear layers rel-L2 0.005 on both scales). Gate-only SFT (100 steps, 32K) for kda (4.77→0.94) and rwkv7 (4.78→0.93) done; gdn2 impossible under grouped value heads (FLA shares f/b per key head). RULER subset (mk2/mq/vt/qa_1 at 16K/64K, 25 samples, corrected protocol): base and both gate-only swaps 100 on all three retrieval tasks at both lengths, qa_1 76–84 (noise). No full SFT (does not fit), no inexact kernels, no distillation, no 131K. The user excluded the 3-seed replication. See docs/framework.md § Second scale. |
